...
namespace mgis::plaxis {
  ...
  struct MFrontBehaviourDataBase {
   ...
   private:
    ...
    MFrontBehaviourDataBase &operator=(const MFrontBehaviourDataBase &) =
        delete;
  };

  const MFrontBehaviourDataBase &MFrontBehaviourDataBase::get() noexcept(
      false) {
  ...
  MFrontBehaviourDataBase::MFrontBehaviourDataBase() noexcept(false) {
    ...
    for (std::string line; std::getline(file, line); ++line_number) {
      ...
      const auto b = load("lib" + tokens[2] + ".so", tokens[1],
                          mgis::behaviour::Hypothesis::TRIDIMENSIONAL);
      auto raise_unsupported_behaviour = [&tokens](std::string_view reason) {
        const auto emsg = "behaviour '" + tokens[1] + "' from library '" +
                          tokens[2] + "' is not supported: ";
        raise(emsg + std::string{reason});
      };
      if (b.btype != Behaviour::STANDARDSTRAINBASEDBEHAVIOUR) {
        raise_unsupported_behaviour(
            "only strain based behaviours are supported");
      }
      if (b.kinematic != Behaviour::SMALLSTRAINKINEMATIC) {
        raise_unsupported_behaviour(
            "only the small strain kinematic is supported");
      }
      if (b.symmetry != Behaviour::ISOTROPIC) {
        raise_unsupported_behaviour("only istropic behavours are supported");
      }
      for (const auto &mp : b.mps) {
        if (mp.type != Variable::SCALAR) {
          raise_unsupported_behaviour(
              "material properties must be scalars (material property '" +
              mp.name + "' must be a scalar, not a '" +
              getVariableTypeSymbolicRepresentation(mp.type_identifier) + "')");
        }
      }
      if (b.esvs.size() != 1) {
        raise_unsupported_behaviour(
            "only behavours having the temperature as their only external "
            "state variable are supported");
      }
      if (b.esvs.at(0).name != "Temperature") {
        raise_unsupported_behaviour(
            "only behavours having the temperature as their only external "
            "state variable are supported");
      }
  ...
  static void copy_diagonal_component(double *const dest,
                                      const double *const src) noexcept {
    dest[0] = src[0];
    dest[1] = src[1];
    dest[2] = src[2];
  }

  static void convert_stress_to_mgis(double *const dest,
                                     const double *const src) noexcept {
  ...
  static void convert_strain_to_mgis(double *const dest,
                                     const double *const src) noexcept {
  ...
  static void convert_stress_from_mgis(double *const dest,
                                       const double *const src) noexcept {
    constexpr auto icste = std::numbers::sqrt2_v<double> / 2;
    copy_diagonal_component(dest, src);
    dest[3] = src[3] * icste;
  ...
  static void convert_tangent_operator_from_mgis(
      double *dest, const double *const src) noexcept {
    constexpr auto icste = std::numbers::sqrt2_v<double> / 2;
    for (int i = 0; i != 6; ++i) {
      for (int j = 0; j != 6; ++j) {
        // transpose
   ...
   * \param[in] isvs0: internal state variables at the beginning of the time
   * step \param[in] deps: strain increment and strain the beginning of the time
   * step \param[in] dt: time increment
   */
  int integrate(mgis::behaviour::IntegrationType it,
    ...
    if ((it == IntegrationType::INTEGRATION_CONSISTENT_TANGENT_OPERATOR) ||
...
int mgis_plaxis_interface_get_material_properties_count(const int model) {
  ...
  try {
    ...
    handle_cxx_exceptions(
        "mgis_plaxis_interface_get_material_properties_count");
  }
  return 0;
}
...
int mgis_plaxis_interface_get_internal_state_variables_count(const int model) {
  ...
  try {
    ...
    handle_cxx_exceptions(
        "mgis_plaxis_interface_get_internal_state_variables_count");
  }
  return 0;
}
 ...
 * \param[out] unsymmetric: integer stating if the tangent operator is
 * unsymmetric \param[out] stressDependent: integer stating if the tangent
 * operator is stress dependent \param[out] timeDependent: integer stating if
 * the tangent operator is time dependent \param[out]
 * constistentTangentOperator:  integer stating if the tangent operator is
 * consistent \param[out] D: tangent operator \param[out] sig: stress tensor at
 * the end of the time step \param[out] isvs: internal state variables at the
 * end of the time step \param[in] mps: material properties \param[in] sig0:
 * stress tensor at the beginning of the time step \param[in] isvs0: internal
 * state variables at the beginning of the time step \param[in] deps: strain
 * increment and strain the beginning of the time step \param[in] dt: time
 * increment
 */
int mgis_plaxis_interface(const int task,
                          const int model,
                          int *const nisvs,
  ...
  try {
    if (task == 1) {
      ...
      const auto n =
          mgis_plaxis_interface_get_internal_state_variables_count(model);
      for (int i = 0; i != n; ++i) {
        isvs[i] = double{};
      }
    } else if (task == 2) {
      constexpr auto it = IntegrationType::INTEGRATION_NO_TANGENT_OPERATOR;
      integrate(it, model, D, sig, isvs, mps, sig0, isvs0, deps, T, dt);
    } else if (task == 3) {
      constexpr auto it =
